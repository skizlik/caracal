#!/bin/bash
# GPU testing script for Caracal Docker container
...
echo -e "\n3. Testing Caracal installation..."
docker run --rm --gpus all -v "$HOST_PROJECTS_DIR":"$CONTAINER_PROJECTS_DIR" $IMAGE_NAME bash -c "
# Create user
groupadd -g $GROUP_ID appuser 2>/dev/null || true
useradd -u $USER_ID -g $GROUP_ID -d /home/appuser -m -s /bin/bash appuser 2>/dev/null || true
...
# Test Caracal installation
echo 'Testing Caracal library installation...'
su - appuser -c 'export PATH=/home/appuser/.local/bin:\$PATH && cd /home/appuser/projects/caracal && pip install --user -e . --no-deps && python3 -c \"import caracal; print(\\\"Caracal imported successfully!\\\")\"'
"
