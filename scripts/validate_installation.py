...
#!/usr/bin/env python3
"""Comprehensive validation of caracal installation and functionality."""
import sys
import os
import time
import json
...
def validate_core_functionality():
    """Test that core caracal features actually work, not just import."""
    ...
    try:
        ...
        # Test 3: Memory info
        memory_info = caracal.get_memory_info()
        print(f"  ✓ Memory tracking works: {memory_info.get('process_rss_mb', 'N/A')} MB")
        
        # Test 4: Statistical functions
        if hasattr(caracal, 'mann_whitney_test'):
            from caracal import mann_whitney_test
...
def generate_report(output_file='caracal_validation.json'):
    """Generate a JSON report of the validation."""
    import caracal
    
    report = {
        'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
        'version': caracal.__version__,
        'features': caracal.get_feature_availability(),
...
def main():
    print_header("CARACAL INSTALLATION VALIDATOR")
    ...
    if missing > 0:
        ...
        print(f"   pip install caracal[all]")
