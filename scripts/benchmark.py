...
#!/usr/bin/env python3
"""Benchmark caracal against vanilla implementations."""
import time
import numpy as np

def benchmark_vs_vanilla():
    """Compare caracal vs vanilla sklearn."""
    print("Caracal vs Vanilla Benchmark\n" + "=" * 40)
    ...
    start = time.time()
    # ... run with caracal
    caracal_time = time.time() - start
    
    print(f"\nResults:")
    print(f"  Vanilla: {vanilla_time:.2f}s (mean acc: {np.mean(vanilla_accuracies):.3f})")
    print(f"  Caracal: {caracal_time:.2f}s (with stats & tracking)")
    print(f"\n✅ Caracal adds {caracal_time - vanilla_time:.2f}s for full analysis")
